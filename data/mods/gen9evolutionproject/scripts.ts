...
export const Scripts: {[k: string]: ModdedBattleScriptsData} = {
	...
	getTeam(options) {
		...
		const randbatsData = (mon: string, initialize: boolean) => { // this is a function so it can be called per-species later as needed!
			...
			if (activeMon.baseStats.spe < 66 && learnset.trickroom && learnset.trickroom.length) {
			...
			const checkRelevance = (
				...
				if (instructions.status) {
					...
					if (baseMove.status) {
						if (typeof baseMove.status === 'string' && instructions.status === baseMove.status) accept = true;
						else if (Array.isArray(baseMove.status) && baseMove.status.includes(instructions.status)) accept = true;
					}
					if (!accept) return false;
				}
				if (instructions.selfBoosts) {
					...
					if (!baseMove.selfBoosts) {
						...
						if (baseMove.secondary && baseMove.secondary.self && baseMove.secondary.self.boosts && (!baseMove.secondary.chance || baseMove.secondary.chance >= 60)) boosts.push(baseMove.secondary.self.boosts);
						else if (refMove && refMove.secondary && refMove.secondary.self && refMove.secondary.self.boosts && (!refMove.secondary.chance || refMove.secondary.chance >= 60)) boosts.push(refMove.secondary.self.boosts);
						
						if (baseMove.secondaries) for (const secondary of baseMove.secondaries) if (secondary.self && secondary.self.boosts && (!secondary.chance || secondary.chance >= 60)) boosts.push(secondary.self.boosts);
						else if (refMove && refMove.secondaries) for (const secondary of refMove.secondaries) if (secondary.self && secondary.self.boosts && (!secondary.chance || secondary.chance >= 60)) boosts.push(secondary.self.boosts);
				...
				if (instructions.targetBoosts) {
					...
					if (!baseMove.targetBoosts) {
						...
						if (baseMove.secondary && baseMove.secondary.boosts && (!baseMove.secondary.chance || baseMove.secondary.chance >= 60)) boosts.push(baseMove.secondary.boosts);
						else if (refMove && refMove.secondary && refMove.secondary.boosts && (!refMove.secondary.chance || refMove.secondary.chance >= 60)) boosts.push(refMove.secondary.boosts);
						
						if (baseMove.secondaries) for (const secondary of baseMove.secondaries) if (secondary.boosts && (!secondary.chance || secondary.chance >= 60)) boosts.push(secondary.boosts);
						else if (refMove && refMove.secondaries) for (const secondary of refMove.secondaries) if (secondary.boosts && (!secondary.chance || secondary.chance >= 60)) boosts.push(secondary.boosts);
			...
			const splitMove = (
				...
				for (const output of outputs) {
					...
					if (output.category) modFragment.category = output.category;
					...
					if (output.acceptedSupport) {
						...
						if (typeof output.acceptedSupport === 'string') {
							if (!modFragment.acceptedSupport.includes(output.acceptedSupport)) modFragment.acceptedSupport.push(output.acceptedSupport);
					...
					if (override) {
						baseMove = modFragment;
						override = false;
					} else destination.push(modFragment);
				}
			...
			const fullySplitMove = (id: string) => {
				...
				for (const fragment of supportSplits) {
					...
					splitMove(fragment, { ability: 'Refrigerate', type: 'Normal', basePower: true, notMoveid: noModifyType, output: { type: 'Ice', basePowerMultiplier: 1.2, tags: 'Refrigerate' }, baseAvoid: 'Refrigerate'} );
					splitMove(fragment, { ability: 'Galvanize', type: 'Normal', basePower: true, notMoveid: noModifyType, output: { type: 'Electric', basePowerMultiplier: 1.2, tags: 'Galvanize' }, baseAvoid: 'Galvanize'} );
					splitMove(fragment, { ability: 'Dragonize', type: 'Normal', basePower: true, notMoveid: noModifyType, output: { type: 'Dragon', basePowerMultiplier: 1.2, tags: 'Dragonize' }, baseAvoid: 'Dragonize'} );
			...
			const makeFragment = (moveSplit: any, instructions: any) => {
				...
				// // label the fragment's relevant attacking stat and defense stat for later use
				// // in VGC, additionally accept support that boosts the relevant attacking stat or lowers the relevant defense stat
				
				// `vgcSupport` -> in VGC, only make fragments using the established "picky VGC support" process
				// // this should now check for priority, spread and pivoting as part of the move fragment as well as the Speed options and Trick Room
				
				// handle Tera Types as appropriate
				// ^ that's currently if the move's power * 1.5 is acceptable but its base power isn't
				
				// it's better to handle minBp in makeFragment instead of checkRelevance because then we can use that information for EVs later
				...
				if (instructions.avoid) {
					...
					else if (Array.isArray(instructions.avoid)) for (const avoid of instructions.avoid) fragment.avoid.push(avoid);
				}
				// if a move is a single-target status move that inflicts a debuff, also tag it as such and avoid other single-target status debuff moves
				if (checkRelevance(fragment, {target: 'normal', targetBoosts: {any: -1}, rejectOther: 'selfSwitch', category: 'Status'})) {
					fragment.avoid.push('statusdebuffmove');
					fragment.tags.push('statusdebuffmove');
				}
				if ((fragment.accuracy && fragment.accuracy < 80 && fragment.accuracy > 1) || checkRelevance(fragment, {other: 'multiaccuracy'})) {
					if (!fragment.buddy) fragment.buddy = {};
					if (!fragment.buddy.roles) fragment.buddy.roles = [];
					fragment.buddy.roles.push('accuracyboost');
			...
			for (const moveSplit of activeData.splitMoves) {
				// failsafes to make sure we have necessary information
				if (!moveSplit.baseMove) return;
				const moveid = this.toID(moveSplit.baseMove);
				const move = this.dex.moves.get(moveSplit.baseMove);
				if (!move) return;
				...
				if (!moveSplit.avoid) moveSplit.avoid = [];
				if (!moveSplit.tags) moveSplit.tags = [];
				...
				})) makeFragment(moveSplit, { role: 'fakeout', vgcSupport: true });
				...
				// // Spread moves
				// All of these only matter for VGC, but there are multiple kinds
				...
				if (checkRelevance(moveSplit, { format: 'vgc', moveid: ['tailwind', 'stickyweb', 'silktrap', 'syrupbomb', 'quash']
				})) makeFragment(moveSplit, { role: 'speedcontrol', avoid: 'speedcontrol', vgcSupport: true });
				// moves that cause paralysis reliably
				if (checkRelevance(moveSplit, { format: 'vgc', status: 'par'
				})) makeFragment(moveSplit, { role: 'speedcontrol', avoid: 'speedcontrol', vgcSupport: true });
				// Bulldoze requires special handling elsewhere
				// moves that directly lower Speed and qualify as VGC support
				if (checkRelevance(moveSplit, { format: 'vgc', notMoveid: 'bulldoze', targetBoosts: {spe: -1}
				})) makeFragment(moveSplit, { role: 'speedcontrol', avoid: 'speedcontrol', vgcSupport: true });
				// moves that directly lower Speed and do solid damage
				if (checkRelevance(moveSplit, { format: 'vgc', notMoveid: 'bulldoze', targetBoosts: {spe: -1}
				})) makeFragment(moveSplit, { role: 'speedcontrol', avoid: 'speedcontrol', minBp: 80 });
				// TODO: mark Venom Drench as needing poison support, but that belongs in splitMove, not here
				...
				})) makeFragment(moveSplit, { role: 'poisonspam' });
				if (checkRelevance(moveSplit, { format: 'vgc', moveid: ['mortalspin', 'poisongas']
				...
				// both sides
				if (checkRelevance(moveSplit, { format: 'vgc', moveid: ['auroraveil', 'followme', 'ragepowder', 'yawn']
				})) makeFragment(moveSplit, { role: ['physicalreduction', 'specialreduction'], vgcSupport: true });
				// physical
				if (checkRelevance(moveSplit, { format: 'vgc', moveid: ['baddybad', 'kingsshield', 'reflect', 'strengthsap']
				})) makeFragment(moveSplit, { role: 'physicalreduction', vgcSupport: true });
				// special
				if (checkRelevance(moveSplit, { format: 'vgc', moveid: ['glitzyglow', 'lightscreen']
				})) makeFragment(moveSplit, { role: 'specialreduction', vgcSupport: true });
				// sleep moves
				if (checkRelevance(moveSplit, { format: 'vgc', status: 'slp'
				})) {
			...
			for (const offeredSupport in pickyVgcSupport) {
				for (const fragment of pickyVgcSupport[offeredSupport]) {
					...
					if (supportFragments.length) {
						if (!activeData.offeredSupport[offeredSupport]) activeData.offeredSupport[offeredSupport] = [];
						for (const supportFragment of supportFragments) activeData.offeredSupport[offeredSupport].push(supportFragment);
					}
				}
			}
			...
			for (const offeredSupport in activeData.offeredSupport) {
				let accepted = false;
			...
			for (const ability of activeData.abilities) {
				...
				switch (ability) {
					...
					case 'Hadron Engine':
						...
						if (!activeData.offeredSupport.antisleep) activeData.offeredSupport.antisleep = [];
					...
					case 'Grassy Surge':
						...
						if (!activeData.offeredSupport.grassyterrain) activeData.offeredSupport.grassyterrain = [];
						activeData.offeredSupport.grassyterrain.push(fragment);
						break;
					case 'Intimidate':
						if (!activeData.offeredSupport.physicalreduction) activeData.offeredSupport.physicalreduction = [];
						activeData.offeredSupport.physicalreduction.push(fragment);
						if (!activeData.offeredSupport.intimidate) activeData.offeredSupport.intimidate = [];
					...
					case 'Misty Surge':
						...
						if (!activeData.offeredSupport.mistyterrain) activeData.offeredSupport.mistyterrain = [];
						activeData.offeredSupport.mistyterrain.push(fragment);
						if (!activeData.offeredSupport.antistatus) activeData.offeredSupport.antistatus = [];
						activeData.offeredSupport.antistatus.push(fragment);
						if (!activeData.offeredSupport.antisleep) activeData.offeredSupport.antisleep = [];
						activeData.offeredSupport.antisleep.push(fragment);
						break;
					case 'Psychic Surge':
					case 'Mega-Neural':
						...
						if (!activeData.offeredSupport.antipriority) activeData.offeredSupport.antipriority = [];
						activeData.offeredSupport.antipriority.push(fragment);
						break;
					case 'Sand Stream':
					case 'Sand Spit':
						...
						if (!activeData.offeredSupport.sand) activeData.offeredSupport.sand = [];
					...
					case 'Snow Warning':
						...
						if (!activeData.offeredSupport.snow) activeData.offeredSupport.snow = [];
						activeData.offeredSupport.snow.push(fragment);
						break;
					case 'Storm Chaser':
						...
						activeData.offeredSupport.rain.push(fragmentElectric);
						activeData.offeredSupport.rain.push(fragmentFlying);
						activeData.offeredSupport.rain.push(fragmentWater);
						activeData.offeredSupport.backuprain.push(fragmentElectric);
						activeData.offeredSupport.backuprain.push(fragmentFlying);
						activeData.offeredSupport.backuprain.push(fragmentWater);
						break;
					// acceptedSupport
					case 'Swift Swim':
					case 'Dry Skin':
		...
		for (const id in this.eligiblePokemon) {
			if (
				...
				(!randbatsData(id).banned || regulationb) && // not banned
		...
		if (team.length) {
			for (const pokemon of team) {
				...
				pokemon.splitMoves = randbatsData(pokemon.species).splitMoves;
			}
		}
		...
		while (firstDraftTeam.length < 6 && eligiblePokemon.length) {
			...
			if (monotype && firstDraftTeam.length === 5) {
				...
				for (const id in this.eligiblePokemon) {
					if (
						!originalTeamSpecies.includes(id) && !originalTeamNumbers.includes(randbatsData(id).num) && // species clause
						!randbatsData(id).banned && // not banned
		...
		for (const draftTeamMember of firstDraftTeam) {
			...
			if (offeredSupportThisStep.length) {
				...
				for (const id of currentStep) {
					...
					let accepted = false;
					for (const role of offeredSupportThisStep) if (randbatsData(id).acceptedSupport[role] && !acceptedSupportThisStep.includes(role)) accepted = true;
			...
			if (offeredSupportThisStep.length) {
				...
				for (const id of currentStep) {
					...
					let accepted = false;
					for (const role of offeredSupportThisStep) if (randbatsData(id).acceptedSupport[role]) accepted = true;
					if (accepted === true) desiredSupport.push(id);
				}
				if (desiredSupport.length) currentStep = desiredSupport;
			}
		...
		if (setsWithoutItems.length) {
			...
			for (const set of setsWithoutItems) {
				...
				if (set.moves.includes('Fling')) { // why would you have this assigned and not already have an item jsmdfg
					if (randbatsData(set.species).types.includes('Dark')) pushItems.push('Iron Ball');
				...
				if (format === 'vgc') {
					...
					if (
						!randbatsData(set.species).types.includes('Ghost') && set.teraType !== 'Ghost' &&
				...
				if (
					...
					if ( // higher priority if you're really not worried about Fake Out
						randbatsData(set.species).types.includes('Ghost') ||
				...
				if ( // let's avoid complete redundancy at least
					format === 'vgc' &&
					!randbatsData(set.species).types.includes('Grass') && set.teraType !== 'Grass' &&
